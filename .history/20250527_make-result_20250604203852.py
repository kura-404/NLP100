...
for i in range(10):
    ...
    if wait_for_batch_completion(batch.id):
        ...
        for line in lines:
            ...
            # 出力保存
            txt_path = f"outputs/{req_id}.txt"
            csv_path = f"outputs/{req_id}_比較.csv"

            if os.path.exists(txt_path) or os.path.exists(csv_path):
                print(f"⚠️ Skipping save: {req_id} already exists.")
                continue
