...
ただし、出力は研究成果概要そのもののみとし、「ご要望があれば〜」のような補足コメントや案内文は含めないでください。
...
if wait_for_batch_completion(batch.id):
    ...
    if not batch_info.output_file_id:
        ...
        for line in lines:
            res = json.loads(line)
            custom_id = res.get("custom_id", "unknown")

            try:
                gen_text = res["response"]["body"]["choices"][0]["message"]["content"].strip()
            except (KeyError, IndexError):
                print(f"⚠️ Skipping {custom_id}: missing choices or content.")
                continue

            examples_used = used_examples_map.get(custom_id, [])
            if not examples_used:
                print(f"⚠️ No examples found for {custom_id}. Skipping.")
                continue
            ...
            txt_path = f"outputs/{dt_str}_{custom_id}.txt"
            csv_path = f"outputs/{dt_str}_{custom_id}_比較.csv"
