...
output_dir = f"outputs/{run_dt_str}"
...
コメントや補足説明は不要です。出力は成果概要の本文のみを含めてください。
...
{prompt_examples}
    ...
    if wait_for_batch_completion(batch.id):
        ...
        if not batch_info.output_file_id:
            ...
            for line in lines:
                ...
                txt_path = f"{output_dir}/{dt_str}_{req_id}_batch{batch_id:02}.txt"
                csv_path = f"{output_dir}/{dt_str}_{req_id}_batch{batch_id:02}_比較.csv"
    ...
    # 次バッチまで3分待機
    if batch_id < 9:
        print(f"⏸️ Waiting 180 seconds before starting next batch...")
        time.sleep(180)
