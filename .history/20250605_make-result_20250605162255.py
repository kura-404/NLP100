...
# 出力用ディレクトリ作成
base_dt_str = datetime.now().strftime("%Y%m%d_%H%M%S")
output_base_dir = f"outputs/{base_dt_str}"
    ...
    if wait_for_batch_completion(batch.id):
        ...
        if not batch_info.output_file_id:
            ...
            for line in lines:
                ...
                txt_path = os.path.join(output_base_dir, f"{base_dt_str}_{req_id}_batch{batch_id:02}.txt")
                csv_path = os.path.join(output_base_dir, f"{base_dt_str}_{req_id}_batch{batch_id:02}_比較.csv")
