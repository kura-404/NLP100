...
def analyze(text):
    ...
    while node:
        ...
        # ここでfeatures[13]が何になっているか確認
        word_type = features[13] if len(features) > 13 else ''  # 語種（UniDic 特有）

        print(f"Word: {word}, Features: {node.feature}, POS: {pos}, WordType (features[13]): '{word_type}'") # デバッグ出力

        if word_type == '漢':
            kango += 1
        elif word_type == '和':
            wago += 1
        # UniDicの場合、外来語は'外'とタグ付けされることがあります。
        # jReadability.netが外来語を漢語・和語に含めているか、除外しているかは不明ですが、
        # 現状のコードでは'外'はカウントされません。
