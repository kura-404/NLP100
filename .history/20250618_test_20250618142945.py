...
# ユーザー辞書のパス（UniDic-MeCab 2.1.2 binのパスに設定）
dic_dir = "/Users/kuramotomana/Desktop/unidic-mecab-2.1.2_bin"
...
def analyze(text):
    ...
    while node:
        ...
        # 語種（UniDic 特有、インデックスは12が正しいと判明済み）
        # このUniDic 2.1.2_binでもfeatures[12]が語種であるか、出力で確認します。
        word_type = features[12] if len(features) > 12 else '' 

        print(f"Word: {word}, POS: {pos}, WordType (features[12]): '{word_type}'")

        # jReadability.netの漢語・和語のカウントに近づけるためのロジックを維持
        # このUniDic 2.1.2_binで「日本」「スクリプト」がどのように分類されるか要確認
        if word_type == '漢':
            kango += 1
        elif word_type == '和':
            wago += 1
