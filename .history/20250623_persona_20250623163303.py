...
].head(100).reset_index(drop=True).copy()  # データを100件に制限
...
for i, row in tqdm(df_filtered.iterrows(), total=len(df_filtered), desc="Processing requests"):
    messages = [
        {"role": "system", "content": "あなたは日本の高齢者です。"},
...
if output_records:
    max_outputs = max(len(record) for record in output_records) - 5  # 行ID, ID, 出現形, 正規形, TREE
else:
    max_outputs = 0

columns = ["行ID", "ID", "出現形", "正規形", "TREE"] + [f"出力{i+1}" for i in range(max_outputs)]
