...
TARGET_FILE = "/Users/kuramotomana/Test/LifeStory_2018_season1.xlsx"  # テスト用
TARGET_COLUMN = "Sadness"          # 例: "Sadness"
TARGET_DIR = "path/to/excel_files"     # 本番用
...
def classify_list(cell_list: list[str]) -> list[str]:
    ...
    try:
        ...
        if len(output) < len(cell_list):
            print(f"⚠️ 出力不足: {len(output)} / {len(cell_list)} 件 → エラーで補完")
            output += ["エラー"] * (len(cell_list) - len(output))
