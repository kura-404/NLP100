...
for file in matched_files:
    ...
    try:
        ...
        # CSV保存
        df_out = pd.DataFrame(counter.items(), columns=["カテゴリ", "件数"]).sort_values(by="件数", ascending=False)
        df_out.to_csv(os.path.join(output_dir, f"{output_prefix}_カテゴリ集計.csv"), index=False, encoding="utf-8-sig")
