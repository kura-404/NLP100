...
# 2. os.getenvで明示的にキーを変数に格納
api_key = os.getenv("OPENAI_API_KEY")

# 3. キーが存在するか確認（推奨）
if not api_key:
    raise ValueError("APIキーが設定されていません。.envファイルに'OPENAI_API_KEY=sk-...'の記述があるか確認してください。")

# 4. OpenAIクライアント初期化時にキーを直接渡す
client = OpenAI(api_key=api_key)
# ------------------------------------------

# 入力CSVファイルと対象列名
input_file = "/Users/kuramotomana/Test/TEST/7件.csv"
