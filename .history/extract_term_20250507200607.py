...
#%%
FILE_NAME = "/Users/kuramotomana/Test/uv-sample-project/テスト用データ10個抜粋.xlsx"
df = pd.read_excel(f"{FILE_NAME}")
# データの準備
ids = df['課題管理番号'].tolist()
research_abstract = df['研究概要'].tolist()
result_abstract = df['成果概要（日本語）'].tolist()

all_texts = research_abstract + result_abstract
...
#outputs = [o.strip("「」") for o  in df_output['yomi_output'].tolist()]
#df['出現形よみ_GPT-4o-mini'] = outputs
df.to_csv(f"{FILE_NAME}_terms.csv", index=False)
df.to_excel(f"{FILE_NAME}_terms.xlsx", index=False)