...
# ========= 出力CSV初期化 =========
header_columns = ["ID", "生成された成果概要（プロンプトによる書き換えbefore）"]
...
for _, row in tqdm(df.iterrows(), total=len(df)):
    ...
    try:
        ...
        revised_summary = res.choices[0].message.content.strip()
    except Exception as e:
        revised_summary = f"エラー発生: {e}"

    row_data = {
        "ID": task_id,
        "生成された成果概要（プロンプトによる書き換えbefore）": revised_summary
    }
